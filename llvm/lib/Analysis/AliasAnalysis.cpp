...
ModRefInfo AAResults::callCapturesBefore(const Instruction *I,
  ...
  for (ImmutableCallSite::arg_iterator CI = CS.arg_begin(), CE = CS.arg_end();
    ...
    if (CS.onlyReadsMemory(ArgNo)) {
      if (R & MRI_Mod) return MRI_ModRef;
      R = MRI_Ref;
      continue;
    }
    // FIXME: not sure if this is even ok, and it should be in the header
    if (CS.dataOperandHasImpliedAttr(ArgNo, Attribute::WriteOnly)) {
    //if (CS.doesNotReadMemory(ArgNo)) {
      if (R & MRI_Ref) return MRI_ModRef;
      R = MRI_Mod;
      continue;
    }
    return MRI_ModRef;
