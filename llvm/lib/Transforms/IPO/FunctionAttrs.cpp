...
namespace {
/// The four kinds of memory access relevant to 'readonly',
/// 'writeonly' and 'readnone' attributes.
...
static MemoryAccessKind checkFunctionMemoryAccess(Function &F, AAResults &AAR,
  ...
  if (!F.hasExactDefinition()) {
    if (AliasAnalysis::onlyReadsMemory(MRB))
      return MAK_ReadOnly;
/*    if (AliasAnalysis::onlyWritesMemory(MRB)) // doesn't exist yet
...
/*    if (I->mayWriteToMemory())
  ...
  if (WritesMemory && ReadsMemory)
    return MAK_MayWrite;
  return ReadsMemory ? MAK_ReadOnly : (WritesMemory ? MAK_WriteOnly : MAK_ReadNone);
}
...
static bool addReadAttrs(const SCCNodeSet &SCCNodes, AARGetterT AARGetter) {
  ...
  for (Function *F : SCCNodes) {
    ...
    AttrBuilder B;
    B.addAttribute(Attribute::ReadOnly).addAttribute(Attribute::ReadNone).addAttribute(Attribute::WriteOnly);
    F->removeAttributes(
    ...
    F->addAttribute(AttributeSet::FunctionIndex,
                    ReadsMemory ? Attribute::ReadOnly : (WritesMemory ? Attribute::WriteOnly : Attribute::ReadNone));
...
/// Returns Attribute::None, Attribute::ReadOnly, Attribute::WriteOnly or Attribute::ReadNone.
...
static bool addArgumentAttrs(const SCCNodeSet &SCCNodes) {
  ...
  for (Function *F : SCCNodes) {
    ...
    for (Function::arg_iterator A = F->arg_begin(), E = F->arg_end(); A != E;
      ...
      if (!HasNonLocalUses && !A->onlyReadsMemory() && !A->hasAttribute(Attribute::WriteOnly)) {
        ...
        if (R != Attribute::None) {
          ...
          Changed = true;
          R == Attribute::ReadOnly ? ++NumReadOnlyArg : (R == Attribute::WriteOnly ? ++NumWriteOnlyArg : ++NumReadNoneArg);
        }
      }
    }
  }
