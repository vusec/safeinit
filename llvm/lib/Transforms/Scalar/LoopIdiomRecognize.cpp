...
private:
  ...
  StoreListMap StoreRefsForInitialized;
  ...
  bool isLegalStore(StoreInst *SI, bool &ForMemset, bool &ForMemsetPattern,
                    bool &ForMemcpy, bool &ForInitialized);
  bool processLoopStores(SmallVectorImpl<StoreInst *> &SL, const SCEV *BECount,
                         bool ForMemset, bool ForInitialized);
  ...
  bool processLoopStridedStore(Value *DestPtr, unsigned StoreSize,
                               ...
                               bool NegStride, bool ForInitialized);
...
bool LoopIdiomRecognize::isLegalStore(StoreInst *SI, bool &ForMemset,
                                      bool &ForMemsetPattern, bool &ForMemcpy, bool &ForInitialized) {
  ...
  // Alyssa says: Let's take advantage of it anyway.
  return true;
  // return false;
}
...
void LoopIdiomRecognize::collectStores(BasicBlock *BB) {
  ...
  for (Instruction &I : *BB) {
    ...
    bool ForInitialized = false;
    // Make sure this is a strided store with a constant stride.
    if (!isLegalStore(SI, ForMemset, ForMemsetPattern, ForMemcpy, ForInitialized))
      continue;
    ...
    if (ForMemset) {
      ...
      assert(ForInitialized);
      Value *Ptr = GetUnderlyingObject(SI->getPointerOperand(), *DL);
      StoreRefsForInitialized[Ptr].push_back(SI);
    }
  }
}
...
bool LoopIdiomRecognize::runOnLoopBlock(
  ...
  for (auto &SL : StoreRefsForInitialized)
    MadeChange |= processLoopStores(SL.second, BECount, true, true);
...
bool LoopIdiomRecognize::processLoopStores(SmallVectorImpl<StoreInst *> &SL,
  ...
  for (unsigned i = 0, e = SL.size(); i < e; ++i) {
    ...
    assert((ForInitialized || FirstSplatValue || FirstPatternValue) &&
    ...
    for (auto &k : IndexQueue) {
      ...
      assert((ForInitialized || SecondSplatValue || SecondPatternValue) &&
             "Expected either splat value or pattern value.");
  ...
  for (SetVector<StoreInst *>::iterator it = Heads.begin(), e = Heads.end();
    ...
    if (processLoopStridedStore(StorePtr, StoreSize, HeadStore->getAlignment(),
                                ...
                                BECount, NegStride, ForInitialized)) {
      // FIXME: is this safe for ForInitialized case?
...
bool LoopIdiomRecognize::processLoopStridedStore(
    ...
    const SCEV *BECount, bool NegStride, bool ForInitialized) {
  Value *SplatValue = isBytewiseValue(StoredVal);
  Constant *PatternValue = nullptr;
  ...
  assert((ForInitialized || SplatValue || PatternValue) &&
         "Expected either splat value or pattern value.");
  ...
  // FIXME: Alyssa rearranged this to avoid unnecesary arithmetic.
  // This seems potentially unsafe (and maybe unneeded now?) but for
  // now it passes all the tests. Should be reviewed (removing it does not
  // seem problematic for anything in the paper?).
  const SCEV *NumBytesS =
      SE->getAddExpr(BECount, SE->getOne(BECount->getType()), SCEV::FlagNUW);
  // i32 is also a valid size for memset, don't bother in that case.. TODO: worth it?
  if (NumBytesS->getType() != Int32Ty || StoreSize != 1) {
    // note that getTruncateOrZeroExtend won't do anything if the type *size* is the same..
    NumBytesS = SE->getTruncateOrZeroExtend(NumBytesS, IntPtr);
  }
  if (StoreSize != 1) {
    NumBytesS = SE->getMulExpr(NumBytesS, SE->getConstant(NumBytesS->getType(), StoreSize),
                               SCEV::FlagNUW);
  }

  Value *NumBytes =
      Expander.expandCodeFor(NumBytesS, NumBytesS->getType() == Int32Ty ? Int32Ty : IntPtr, Preheader->getTerminator());

  CallInst *NewCall;
  if (ForInitialized) {
    // FIXME: do this in the irbuilder
    // FIXME: do we really care if it's i8* or not
    Value *Ptr = BasePtr;
    PointerType *PT = cast<PointerType>(BasePtr->getType());
    if (!PT->getElementType()->isIntegerTy(8)) {
      // Otherwise, we need to insert a bitcast.
      PT = Builder.getInt8PtrTy(PT->getAddressSpace());
      BitCastInst *BCI = new BitCastInst(BasePtr, PT, "");
      Builder.GetInsertBlock()->getInstList().insert(Builder.GetInsertPoint(), BCI);
      Builder.SetInstDebugLocation(BCI);
      Ptr = BCI;
    }

    Value *Ops[] = { Ptr, UndefValue::get(Builder.getInt8Ty()), NumBytes, Builder.getInt32(StoreAlignment), Builder.getInt1(false) };
    Type *Tys[] = { Ptr->getType(), NumBytes->getType() };
    Module *M = Preheader->getParent()->getParent();
    Value *TheFn = Intrinsic::getDeclaration(M, Intrinsic::initialized, Tys);
 
    NewCall = CallInst::Create(TheFn, Ops);
    Builder.GetInsertBlock()->getInstList().insert(Builder.GetInsertPoint(), NewCall);
    Builder.SetInstDebugLocation(NewCall);
    NewCall->setDebugLoc(TheStore->getDebugLoc());
//    dbgs() << "*** INSERTED INITIALIZATION " << *NewCall << " from store to " << *Ev << " at " << *TheStore << "\n";
    return true;
  } else if (SplatValue) {
    NewCall =
